...
import com.example.demo.entity.UserForm; // 確保 UserForm 包含 email, passwd, name 屬性
...
public class LoginController {
    ...
    public String verifyCodeAndProceed(@RequestParam String code, // ***關鍵修正：不再接收 @RequestParam String email***
        ...
        if (verificationService.verifyCode(email, code)) { // 使用從 Session 中取出的 Email
            ...
            logger.info("Verification code successful for email: {}. Redirecting to name/password input.", email);
    ...
    public String showNamePasswordRegistrationPage(HttpSession session, Model model) {
        ...
        if (verifiedEmail == null) {
            logger.warn("Access to name/password registration page without verified email in session.");
    ...
    public String completeRegistration(@ModelAttribute("userForm") @Valid UserForm userForm,
        ...
        try {
            ...
            boolean registered = memberService.registerUserAfterVerification(verifiedEmail, userForm.getPasswd(), userForm.getName());
            if (registered) {
