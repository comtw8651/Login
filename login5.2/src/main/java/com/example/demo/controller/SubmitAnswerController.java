...
public class SubmitAnswerController {
    ...
    public ResponseEntity<Map<String, Object>> submitAnswer(HttpServletRequest request) {
        ...
        try (BufferedReader reader = request.getReader()) {
            ...
            // 從 JSON 取得資料時，進行型別轉換和錯誤處理
            int userId = jsonObject.get("userId").getAsInt();

            // videoId 從 String 轉換為 Long
            // 注意：這裡假設 videoId 在 JSON 中是 String，但 Answer 物件需要 Long
            String videoIdStr = jsonObject.get("videoId").getAsString();
            Long videoId;
            try {
                videoId = Long.parseLong(videoIdStr);
            } catch (NumberFormatException e) {
                response.put("success", false);
                response.put("message", "❌ videoId 格式錯誤，必須是數字字串：" + videoIdStr);
                System.err.println("Error parsing videoId: " + videoIdStr + " to Long. " + e.getMessage());
            ...
            for (int i = 0; i < answersArray.size(); i++) {
                ...
                // 取得正確答案索引，並比對是否正確
                // 確保 getCorrectAnswerIndex 方法存在並正確處理
                ...
                // 1. setUserId: int -> Long
                answer.setUserId((long) userId); // userId 是 int，setUserId 接受 Long，所以轉換

                // 2. setVideoId: String -> Long (已在上方處理)
                answer.setVideoId(videoId); // videoId 現在是 Long 型別了

                answer.setAttemptId(attemptId);
                answer.setQuizId((long) quizId); // quizId 是 int，但如果 Answer 的 setQuizId 接受 Long，則需轉換
            ...
            // 注意：insertQuizResult 方法中的 videoId 參數也需要是 Long
            try {
